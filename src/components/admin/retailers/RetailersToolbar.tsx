...
/**
 * Search box + Add Retailer button. The only genuinely client-side state
 * in this whole module is the debounced search text below (mirrors the
 * "minimal client state" isolation already used for
 * DashboardDateFilterControl) — search itself is server-side (this task's
 * explicit requirement), reached via a URL navigation, not a client fetch.
 */
...
export default function RetailersToolbar({ initialSearch }: RetailersToolbarProps) {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="relative flex-1 sm:max-w-sm">
        ...
        <label htmlFor="retailers-search" className="sr-only">
          Search retailers
        </label>
        <input
          id="retailers-search"
