 ...
 * every other module's pagination component. */
...
export default function AuditLogPagination({ page, pageSize, totalCount, search }: AuditLogPaginationProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-3 border-t border-warm-200 bg-warm-50 px-4 py-3 sm:flex-row">
      <p className="text-xs text-brown-500">
        {totalCount === 0 ? "No entries" : `Showing ${from}–${to} of ${totalCount} entries`}
      </p>
      <nav aria-label="Audit Log pagination" className="flex items-center gap-3 text-sm text-brown-600">
        {page > 1 ? (
